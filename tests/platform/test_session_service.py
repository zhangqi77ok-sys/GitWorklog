...
def test_delete(db_session: Session) -> None:
    conv = service.get_or_create_conversation(db_session, user_id=1)
    service.append_message(db_session, conv.conversation_id, "user", "hi")
    service.delete_conversation(db_session, conv.conversation_id)
    assert len(service.list_conversations(db_session, 1)) == 0
    assert len(service.get_messages(db_session, conv.conversation_id)) == 0
