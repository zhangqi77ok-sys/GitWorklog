...
### 1. 左侧工作台双栏架构 (2-Pane Projects & Session Workspace)
* **上栏：多项目树状聚合与会话管理 (Projects & Sessions Management)**：
  * 支持多本地工程并行打开，每个项目聚合各自独立的会话记录；首次载入新项目时自动初始化【会话 1 (默认)】；
  * 支持全/局部即时搜索 (`Ctrl+K`)、多彩色自定义标签 (`#架构重构`, `#Bug修复`, `#单测`)、会话置顶锁定 (`Pin to Top`)、行内双击重命名与删除；
* **下栏：当前项目/会话文件目录树 (Current Project / Session File Explorer)**：
  * 常驻下半区独立滚动视窗，精准展示当前激活项目或当前会话关联的完整工作区目录树；
  * 支持文件夹展开/折叠、状态识别与快速刷新；
* **全工作区联动**：点击上栏任意项目或会话，下栏目录树、中央 Agent Chat 对话流、右侧 Monaco 编辑器与底部终端工作目录（CWD）毫秒级自动对齐。
