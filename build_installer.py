...
def build() -> Path:
    print(f"=== [Tcode v{VERSION}] Starting Incremental Release Build Pipeline ===")
    RELEASE_DIR.mkdir(parents=True, exist_ok=True)

    # 1. Build & Test Frontend
    print("\n[1/5] Building frontend production assets...")
    run(frontend_npm_command("run", "build"), cwd=ROOT)
    print("\n[2/5] Running Vitest unit tests...")
    run(frontend_npm_command("test"), cwd=ROOT)
    ...
    # 4. Build Tcode-Setup-v2.0.0.exe
    ...
    print(f"[OK] Successfully generated Windows compat installer: {COMPAT_INSTALLER_OUTPUT} ({COMPAT_INSTALLER_OUTPUT.stat().st_size:,} bytes)")
