...
export const ChatColumn: React.FC<ChatColumnProps> = ({
  ...
  // Interactive Human-in-the-Loop Action Approval Queue
  ...
  // Extract pending actions from incoming messages
  useEffect(() => {
    if (sessionAutoAllowed || permissionPolicy === 'autonomous_agent') return;
    
    // Scan messages for unhandled actions
    const newActions: AgentPendingAction[] = [];
    messages.forEach(msg => {
      if (msg.role !== 'assistant') return;
      const content = msg.content || '';
      if (!content.includes('```write_file:') && !content.includes('```run_command') && !content.includes('```file:')) return;
      ...
      for (let i = 0; i < lines.length; i++) {
        ...
        if (trimmed.startsWith('```')) {
          if (inCode) {
            ...
            const isCmd = ['run_command', 'bash', 'sh', 'powershell', 'cmd'].includes(cleanLang.toLowerCase());

            if (isWrite && targetPath) {
              const actId = `act-${msg.id}-${targetPath}`;
  ...
  // Execute a single action on host
  const executePendingAction = async (action: AgentPendingAction) => {
    if (action.type === 'write_file') {
      try {
        ...
        setChangesetToast(`✨ 已成功将代码写入: ${action.target}`);
      } catch (e) {
        setChangesetToast(`❌ 写入 ${action.target} 失败`);
      }
    } else {
      try {
        ...
        setChangesetToast(`▶️ 终端指令已在后台执行完成`);
      } catch (e) {
        setChangesetToast(`❌ 终端指令执行异常`);
      }
    }
  ...
  const handleRejectOnce = (action: AgentPendingAction) => {
    setProcessedActionIds(prev => new Set(prev).add(action.id));
    setPendingActionQueue(prev => prev.filter(a => a.id !== action.id));
    setChangesetToast(`🛑 已跳过执行: ${action.target}`);
  ...
  const handleAllowAllInSession = async (action: AgentPendingAction) => {
    ...
    setChangesetToast(`⚡ 已允许当前会话全部执行！共落地 ${queueToRun.length} 项操作。`);
