...
## 🏷️ 九、三态会话作用域与常驻状态标签体系 (Three-Tier Session Scope & Pinned Context Badges)

### 9.1 核心痛点与设计意图
在传统的 AI 编程工具中，开发者往往产生强烈的**“上下文迷失感”**：
- *“我当前这段对话，AI 究竟知不知道我当前在哪个工程里？”*
- *“我只是想单聊某个具体函数，AI 却老是把整个项目的文件翻个底朝天，浪费大量 Token。”*
- *“我想随手问个通用语法算法，AI 却强行加载了几十个工程规则与文件树。”*

为此，CodeMind-Hub 正式确立**“三态会话作用域模型”**，并在对话柱顶部与左侧会话树上提供**醒目固定的常驻标记标签 (Fixed Status Badges)**：
...
```
         ...
         ┌──────────────────────────────────────┼──────────────────────────────────────┐
         ▼                                      ▼                                      ▼
【状态 1: 🌐 全局自由会话】              【状态 2: 📁 工程级会话】              【状态 3: 📄 文件专精会话】
- 零工程绑定，纯轻量对话                - 绑定整个项目根目录                   - 绑定具体单个文件 (带行号)
- 快速问答、算法设计、通用语法          - 自动注入项目根规则、Git分支与AST拓扑 - 自动注入该文件全文与AST骨架
- 标记: [ 🌐 全局自由会话 ]              - 标记: [ 📁 项目: agent-learning ]    - 标记: [ 📄 文件: GatewayBus.ts ]
- 可中途一键“升级绑定至工程”            - 支持多文件协同落盘与统一 Diff 审批   - 精准局部重构，节省 80% Token
```
...
#### 1. 🌐 全局自由会话 (Global Free Session)
- **触发方式**：打开应用无需选择项目/文件，直接在输入框提问，或点击左侧 `[ ➕ 开启新自由会话 ]`；
- **常驻固定标签**：对话区顶部常驻浅米灰色胶囊：
  `[ 🌐 全局自由会话 · 无工程绑定 ]`（右侧附带小链接 `[ ➕ 关联到当前工程 ]`）；
- **上下文行为**：不加载任何特定项目的文件树与 AST，纯粹通用推理，冷启动 0 延迟，极速轻量；
- **升档流转**：聊到一半用户觉得需要落地到当前工程时，点击 `[ 关联到工程 ]`，瞬时无缝升级为工程级会话。

#### 2. 📁 工程级会话 (Project Scope Session)
- **触发方式**：在左侧已打开的工程目录下点击 `[ ➕ 新建工程会话 ]`，或在项目根目录右键创建；
- **常驻固定标签**：对话区顶部常驻陶土暖橙色边框胶囊：
  `[ 📁 项目: agent-learning (🌿 main) · 32个文件 ]`；
- **上下文行为**：自动挂载当前工程的 `.cursorrules`、`PROJECT_RULES.md`、Git 影子快照引擎与 AST 关系雷达；
- **主要使命**：负责多文件跨模块重构、功能新增、系统级端到端测试与架构演进。

#### 3. 📄 文件专精会话 (File-Focused Session)
- **触发方式**：在文件树右键具体文件选择 `[ 💬 针对此文件提问/重构 ]`，或在编辑器选中代码块按下快捷键；
- **常驻固定标签**：对话区顶部常驻高对比蓝色/暖橙双色精致胶囊：
  `[ 📄 文件: src/bus/GatewayBus.ts (L1-L120) · 240行 ]`（点击标签瞬时在右侧编辑器高亮联动聚焦）；
- **上下文行为**：上下文严格锁定该文件本身及其 AST 骨架，默认拦截对其他非关联文件的破坏性改动；
- **主要价值**：单文件精雕细琢、单函数算法优化、单组件 Bug 修复，**由于无需传递全工程无关信息，单次推理立省 80% Token**！

---

### 9.3 左侧会话树的三态层次化归集

在左侧面板的会话历史中，以清晰的视觉层级和副标题固定标签直观归集：

```
📁 SESSIONS & CHATS
├─ 🌐 全局会话 (Global)
│  └─ 💬 Python 3.12 模式匹配语法差异 · 4 msgs
├─ 📁 工程: agent-learning
│  ├─ 🏷️ [项目] 重构三栏自适应流体布局 · 18.5k tokens
│  ├─ 🏷️ [项目] 接入 Gemini / DeepSeek 网关子线 · 12.1k tokens
│  └─ 📄 文件级会话
│     ├─ 🏷️ [GatewayBus.ts] 优化事件分发防重机制 · 4.2k tokens
│     └─ 🏷️ [OptionsCard.tsx] 增加自定义输入补充框 · 3.1k tokens
```
