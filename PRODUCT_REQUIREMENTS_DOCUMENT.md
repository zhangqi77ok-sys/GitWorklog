...
## 🖥️ 八、桌面端动态区块形变与全场景人机交互规范 (Desktop Fluid Workspace & Interaction Ergonomics)

真正顶级的桌面 IDE，其核心壁垒不仅在于底层算法，更在于**极度顺滑、没有顿挫感、符合直觉的流体人机工程交互 (Fluid Ergonomics)**。
以下规范由 **Agent 产品经理** 与 **UI/UX 体验设计师** 深度推演制定：

---

### 8.1 三栏流体拉伸与自适应吸附机制 (Fluid Panes & Snap Dynamics)

```
┌──────┬──────────────────────┬────────────────────────┬──────────────────────────────────────┐
│ 42px │   左栏: 会话与文件   │    中栏: 对话推理流    │        右栏: 代码预览/产物画布       │
│ 活动 │  [200px ~ 400px]     │   [360px ~ 650px]      │           [弹性自适应 45%+]          │
│ 栏   │  双击分界线折叠/还原 │   双击分界线自动均分   │        右下角集成 28px 终端抽屉把手  │
└──────┴──────────────────────┴────────────────────────┴──────────────────────────────────────┘
```

1. **分栏拖拽与临界值保护 (Draggable Splitters)**：
   - **左侧面板 (LeftPanel)**：默认 `240px`，最小 `180px`，最大 `400px`；小于 `140px` 自动触发**磁吸折叠**；
   - **中间对话柱 (ChatColumn)**：默认弹性 `45%`，最小 `320px`，最大 `700px`；保证代码块与 Diff 始终有舒适阅读宽度；
   - **右侧工作区 (EditorWorkspace)**：占剩余全量空间（通常 `50%~60%`），设置保底最小 `400px`；
2. **快捷分屏手势与双击重置**：
   - 双击“左/中”分界线：一键完全折叠侧边栏（等同于快捷键 `Ctrl/Cmd + B`）；
   - 双击“中/右”分界线：自适应重置为黄金比例（45% : 55%）；
   - 分界线鼠标悬浮时呈现 `1px` 陶土暖橙光晕（`#D96B27`），光标变为 `col-resize`，拖拽时有高帧率平滑跟随（Zero Jitter）。

---

### 8.2 文件点击与临时预览状态机 (File Tabs & Ephemeral Preview)

```
[ 单击文件树项 / 单击聊天文件链接 ] ──> 开启【临时倾斜预览标签】(Italic Preview Tab: 📄 App.tsx)
                                          │
                  ┌───────────────────────┴───────────────────────┐
                  ▼                                               ▼
         [ 单击另一个新文件 ]                             [ 双击该标签 / 产生代码编辑 / 点击固钉]
                  │                                               │
                  ▼                                               ▼
   复用当前临时标签 (不污染标签栏)                 固化为【正式常驻标签】(Regular Tab: 📄 App.tsx)
```

1. **临时标签保护机制 (Ephemeral Preview)**：
   - 单击文件树或 AI 输出中的 `[App.tsx](file://...)`：右侧工作区在临时标签页打开，标签文字为*斜体*；
   - 连续单选查看不同文件时，**复用同一临时标签**，杜绝打开十几个无用标签把顶部挤爆；
2. **精准行号跳转微动画 (Scroll-to-Line)**：
   - 若链接包含行号（如 `file:///App.tsx#L45-L60`）：编辑器自动平滑滚动至第 45 行居中，并在对应行块淡入 `2 秒柔和陶土橙半透明高亮光晕`，随后自然淡出；
3. **标签溢出流体滚动**：
   - 顶部标签栏支持鼠标滚轮横向平滑滚动，右侧常驻 `...` 更多标签下拉菜单与一键关闭所有标签快捷操作。

---

### 8.3 多模态产物画布与分屏透视 (Multimodal Artifact Canvas: Doc / PDF / Image / Video)

当 AI 生成非纯文本代码的富媒体产物（Markdown 文档、PDF 报告、Mermaid 架构图、UI 生成图、录屏演示）时，**坚决不挤在窄小的聊天气泡里，而是联动右侧视界展开多模态画布**：

```
                    ┌───────────────────────────────────────────────┐
                    │               右侧多模态产物画布               │
                    ├──────────────────────┬────────────────────────┤
                    │   Tab 1: 📄 代码编辑 │   Tab 2: 🖼️ 产物画布   │
                    └──────────────────────┴────────────────────────┘
                                           │
         ┌──────────────────┬──────────────┴──────────────┬──────────────────┐
         ▼                  ▼                             ▼                  ▼
  【Markdown / 文档】  【Mermaid 架构图谱】        【图片 / UI 原型图】    【视频 / 录屏动图】
  - 沉浸式排版排版     - SVG 矢量平滑缩放/拖拽平移  - 像素级 100% 细节对比  - 24fps 循环回放
  - 支持目录大纲导航   - 一键导出 PNG / SVG         - 左右原图 vs 现图分屏  - 逐帧播放与跳帧审查
  - 一键导出 PDF / Doc - 双击节点高亮引用代码       - 支持取色器与尺寸标尺  - 支持画中画微缩窗
```

1. **分屏透视模式 (Side-by-Side Split View)**：
   - 点击右上角 `[ ◫ 分屏对照 ]`，右侧工作区瞬切为“左右两半”：左半部分显示源文件，右半部分显示渲染后的 Markdown/PDF/图片画布，修改代码实时热更新产物！
2. **图表与架构图原生可交互**：
   - Mermaid 或知识图谱不是死图片，支持鼠标滚轮缩放、右键平移拖拽、点击特定模块瞬间过滤上下文。

---

### 8.4 ReAct 任务执行计划的“呼吸伸缩”架构 (Task Plan Breathing & Floating Capsule)

长任务往往包含多个子步骤（扫描 -> 规划 -> 补丁 -> 测试 -> 验证）。执行计划的展示形态必须兼顾**“宏观全局掌控”**与**“微观不占视野”**：
...
【阶段 B: 极简悬浮收缩态 (Floating Progress Capsule)】
 ...
 └─────────────────────────────────────────────────────────────┘
                               │
            【点击胶囊任何区域平滑拉开抽屉，零跳转查看详情】
```

1. **动态高度自适应**：
   - 任务生成时：作为结构化卡片完整展开在对话流中；
   - 任务开始执行后：平滑折叠为固定高度 `28px` 的**半透明悬浮胶囊**，固定在输入框工具栏上方，展示当前进度百分比与当前步骤脉冲动画；
2. **零阻断抽屉悬停 (Flyout Drawer)**：
   - 开发者需要查看全局任务状态时，只需鼠标悬浮或点击进度胶囊，自底部平滑滑出半透明半高遮罩面板，查看详情后移开鼠标自然收起，**完全不打乱当前代码阅读位置**。

---

### 8.5 抽屉式集成终端与交互微动效 (Terminal Drawer & Spring Transitions)

1. **抽屉终端的升降手势**：
   - 右侧工作区底部保留 `24px` 极简暖炭黑把手条，标有 `📟 终端 (zsh / pwsh) · 运行正常`；
   - 单击把手瞬时以弹簧曲线（`cubic-bezier(0.16, 1, 0.3, 1)`）平滑升起至默认 `220px` 高度；
   - 边缘支持任意上下拖动调节高度，向上甩动全屏铺满，向下轻拉自然隐藏；
2. **动效哲学（物理质感、克制、无延迟）**：
   - 所有动画持续时间严格控制在 `150ms ~ 220ms`，杜绝任何拖沓的慢速装饰动画；
   - 颜色切换使用柔和渐变过渡，消除界面刺眼闪烁，全面匹配企业级专业 IDE 的极客质感。
