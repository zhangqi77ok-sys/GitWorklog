...
## 🏷️ 九、两级会话架构与三态常驻标签体系 (Two-Tier Session Architecture & Scope Badges)

### 9.1 架构分层体系：一级三态作用域，二级具体会话实例
系统正式确立**两级树状会话导航与上下文模型**：
- **一级为【三态作用域大类 (Tier-1 Scope Category)】**：界定 AI 上下文的作用域范围与安全边界；
- **二级为【具体会话实例 (Tier-2 Concrete Session Instances)】**：用户在对应作用域下创建的真实对话任务。
...
### 9.2 一级三态作用域的详细定义与固定标记

#### 一级大类 1：🌐 全局自由会话 (Global Free Scope)
- **定位**：无需选择项目、无需选择文件，随时打开即聊；
- **一级固定标记**：对话柱与左侧栏常驻标明 `[ 🌐 全局自由会话 ]`；
- **核心价值**：快速问答、跨语言选型、算法设计、写独立小工具脚本，0 延迟、0 工程噪声；
- **升档机制**：对话中可随时点击 `[ ➕ 升级关联至当前工程 ]`，一键无缝沉淀为工程级会话。

#### 一级大类 2：📁 工程级会话 (Project Scope)
- **定位**：先选定目标工程目录（如 `agent-learning`），在该工程下创建的具体对话；
- **一级固定标记**：对话柱常驻高对比陶土暖橙胶囊 `[ 📁 工程: agent-learning (🌿 main) ]`；
- **核心价值**：自动注入当前工程的 `.cursorrules`、`PROJECT_RULES.md`、全工程 AST 关系网与 Git 影子快照；
- **适用场景**：跨模块多文件重构、系统级新特性研发、架构重构与端到端测试。

#### 一级大类 3：📄 文件专精会话 (File-Focused Scope)
- **定位**：先在工程中选定某个具体文件（如在文件树或编辑器右键 `[ 针对此文件开新会话 ]`），针对该文件创建的微粒度专属会话；
- **一级固定标记**：对话柱常驻精致蓝色/暖橙固定胶囊 `[ 📄 文件: src/bus/GatewayBus.ts (L1-L120) ]`；
  - *联动特性*：点击该标记，右侧代码工作台瞬时平滑高亮聚焦到对应文件与代码行；
- **核心价值**：上下文仅锁定当前目标文件及必要接口骨架，杜绝全工程无关代码污染，**单次推理立省 80%+ Token**！

---

### 9.3 界面两级导航与面包屑交互规范

1. **左侧面板一级三态分段器 (Tier-1 Segmented Switch)**：
   - 会话面板顶部提供极简三段选择胶囊：
     `[ 🌐 全局 (3) ] | [ 📁 工程: agent-learning (12) ] | [ 📄 文件 (5) ]`；
   - 切换一级标签，下方二级会话列表实时平滑过滤呈现，清爽整洁；
2. **对话柱顶部常驻两级面包屑导航 (Two-Tier Breadcrumb Navigation)**：
   - 顶栏固定呈现清晰的两级位置指引：
     - 全局会话：`🌐 全局自由会话 › 💬 Python 3.12 模式匹配讨论`
     - 工程会话：`📁 工程: agent-learning (🌿 main) › 💬 重构三栏自适应流体布局`
     - 文件会话：`📄 文件: src/bus/GatewayBus.ts › 💬 优化事件分发防重机制`
   - 点击一级面包屑可快速呼出切换下拉，点击二级面包屑可一键就地重命名会话标题。

---

## 💡 十、真实 Coding 痛点洞察与八大进阶功能 (Real-World Developer Pain Points)
...
| **P0 (Must-Have)** | **三态会话作用域与常驻标签** | 支持全局自由聊/工程级/文件级专精会话，顶部常驻固定标记与左侧归集 |
