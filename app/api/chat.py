...
async def chat_sse(req: ChatRequest, session: DbDep):
    ...
    async def event_generator():
        ...
        mesh = get_mesh_engine()
        steps = await mesh.execute_pipeline(req.query, req.conversation_id, req.provider, req.model)
        ...
        for step in steps:
            ...
            if "code" in step:
                get_graph_service().record_change(req.conversation_id, "app/utils/task_solver.py", "MODIFY", "实现业务算法")
