...
- Phase 3.5 已将策略中心接入桌面 IPC 和 SQLite：保存的默认策略会用于新建 LoopRun 与后续分析审核门控
...
- Phase 3.5 已将规则启用状态和优先级保存到桌面数据库，浏览器预览仍回退到 localStorage
...
- Phase 3.3 已在 Web 控制台加入独立审核队列工作页：左侧队列、右侧详情，操作仍在当前窗口完成
