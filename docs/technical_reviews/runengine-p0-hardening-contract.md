...
| `GET http://127.0.0.1:8010/` | HTTP 200??? HTML?867B? |

### 4.2 ????????? Key ???????????
????? `/api/proxy` ? OpenCode Zen ?????

| ?? | ?? |
| --- | --- |
| `GET https://opencode.ai/zen/v1/models`?Bearer? | HTTP 200?5286B ????? |
| `POST .../chat/completions` `mimo-v2.5-free`????? | HTTP 200??? `chat.completion`?usage: 252/16/268 tokens?cost:"0"? |
| `POST .../chat/completions` `mimo-v2.5-free`?`stream:true`? | HTTP 200?`text/event-stream`?24 chunks / 13 ? `data:` ????? `[DONE]` ?? |
| `deepseek-v4-flash` / `gpt-5.1-codex` | HTTP 401 `CreditsError: Insufficient balance`?Key ??????????/??????????? |
| ??????????? | ????????????????? |

### 4.3 ????????????
- ?? PowerShell `curl.exe` ? JSON ? `model` ??????? ? ??? `Model  is not supported`?
  ?? Node `fetch(JSON.stringify(body))` ????????????????**???????????????????**

### 4.4 ????????
- `git grep -E "sk-[A-Za-z0-9]{20,}"` ?? `prototype/src`?`src-desktop`?`scripts`???? `sk-` ????
- `tests/credentialHygiene.test.ts` ?????????? 3 ??????????????
