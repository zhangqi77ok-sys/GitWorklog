...
## 目标
每次原型或宿主代码变更完成后，使用可复现的 PyInstaller 链路生成根目录 `release/Tcode-Setup.exe`。安装器必须嵌入本次构建的前端静态资源和桌面宿主，不得复用历史二进制文件。
...
5. 以 `src-desktop/setup_wizard.py` 生成窗口化单文件安装器 `release/Tcode-Setup.exe`。
